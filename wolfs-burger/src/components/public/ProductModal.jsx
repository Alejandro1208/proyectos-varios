...
const ProductModal = ({ product, onClose }) => {
    ...
    return (
        ...
        >
            <div 
                // --- CAMBIO CLAVE AQUÍ: Aumento el max-w y el width en desktop ---
                className="bg-white rounded-lg shadow-xl max-w-lg md:max-w-5xl w-full max-h-[90vh] flex flex-col md:flex-row overflow-hidden" 
                onClick={e => e.stopPropagation()} 
            >
                {/* Galería de Imágenes */}
                <div 
                    // --- CAMBIO CLAVE AQUÍ: La imagen ocupa más espacio en desktop ---
                    className="w-full md:w-3/5 bg-gray-100 relative"
                >
                    {product.images && product.images.length > 0 ? (
                        <>
                            <img src={product.images[currentImageIndex].image_url} alt={product.name} className="w-full h-64 md:h-full object-contain md:object-cover"/>
                ...
                <div 
                    // --- CAMBIO CLAVE AQUÍ: El contenido del texto ocupa menos espacio en desktop ---
                    className="w-full md:w-2/5 p-6 flex flex-col overflow-y-auto"
                >
