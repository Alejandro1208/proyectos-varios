        ...
        {banners && banners.length > 0 ? (
            banners.map((banner, index) => (
              ...
              >
                ...
                <picture>
                  {/* Si hay imagen móvil y la pantalla es pequeña (max 768px), se usa la imagen móvil */}
                  {banner.image_url_mobile && (
                    <source media="(max-width: 768px)" srcSet={banner.image_url_mobile} />
                  )}

                  {/* Para pantallas más grandes (o si no hay imagen móvil), se usa la de escritorio */}
                  <source media="(min-width: 769px)" srcSet={banner.image_url} />

                  {/* Imagen por defecto si el navegador no soporta <picture> o para el resto de los casos */}
                  <img src={banner.image_url} alt={banner.alt_text || 'Banner image'} className="w-full h-full object-cover" />
                </picture>
              </div>
            ))
        ) : (
