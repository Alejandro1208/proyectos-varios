...
const AdminAppearance = () => {
  ...
  const [colors, setColors] = useState({
    ...
    color_accent: '#f39c12',
    color_header_bg: '#FFFFFF', // <-- 1. Añadimos el nuevo color al estado
  });
  ...
  useEffect(() => {
    if (siteSettings) {
      setColors({
        ...
        color_header_bg: siteSettings.color_header_bg || '#FFFFFF', // <-- 2. Cargamos el color guardado
      });
    }
  }, [siteSettings]);
      ...
      <div className="bg-white shadow rounded-lg">
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {/* --- 3. AÑADIMOS EL NUEVO CAMPO PARA EL MENÚ --- */}
            <div className="flex items-center gap-4">
              <input type="color" name="color_header_bg" value={colors.color_header_bg} onChange={handleColorChange} className="w-12 h-12 rounded-full border-gray-300" />
              <div>
                <label className="block text-sm font-medium text-gray-700">Color del Menú (Header)</label>
                <p className="text-xs text-gray-500">Fondo de la barra de navegación. El texto cambiará a blanco/negro automáticamente.</p>
              </div>
            </div>
